...
**작성 원칙**:
- 테스트별로 별도 디렉토리 생성
- 디렉토리명: `YYYY-MM-DD_테스트-이름`
- README.md는 필수
- 스크린샷, 메트릭 등은 선택적으로 포함
- 테스트 결과는 삭제하지 않고 보관 (품질 기준선 유지)
...
- [ ] activeContext.md가 현재 상태를 정확히 반영하는가?
- [ ] 완료된 작업은 progress.md에 기록되었는가?
- [ ] 기술적 결정이 있었다면 techContext.md에 기록되었는가?
- [ ] 데이터 흐름 변경이 있었다면 dataflow.md에 반영되었는가?
- [ ] 테스트를 수행했다면 test_results/에 결과를 저장했는가?
- [ ] 프로젝트 목표나 범위가 변경되었다면 projectBrief.md를 업데이트했는가?
- [ ] 같은 정보가 여러 파일에 중복 기록되지 않았는가?
- [ ] 모든 내용이 한글로 작성되었는가? (코드/파일명 제외)
- [ ] 날짜 형식이 올바른가? (YYYY-MM-DD)
- [ ] progress.md는 최신 항목이 상단에 있는가? (시간 역순)

---

## 일반 원칙

1. **한 곳에 한 번만**: 같은 정보를 여러 파일에 중복 기록하지 않습니다.
2. **적절한 위치**: 각 정보를 가장 적합한 파일에 기록합니다.
3. **한글 작성**: 모든 설명과 내용은 한글로 작성합니다.
4. **시간 정보**: 날짜는 YYYY-MM-DD 형식을 사용합니다.
5. **상세함과 간결함의 균형**: 필요한 정보는 모두 포함하되, 불필요하게 장황하지 않게 합니다.
6. **업데이트 날짜 명시**: 각 파일의 최종 업데이트 날짜를 명시합니다.
